...
Embedded real-time systems (RTS) are all around us --- they are used to monitor and control physical systems and processes in various domains such as unmanned vehicles, self-driving cars, avionics, critical infrastructures, manufacturing systems, medical devices, to name but a few.
...
Until recently, security was not an essential concern in the design of
such systems since they used proprietary protocols, platforms, and software
and were not connected to the outside world. Recent attacks have shown that
such critical systems are vulnerable to malicious attacks, and such adversarial actions can result in severe  damage to humans, the systems, and the environment. Integrating
security in such systems is not an easy task since RTS have stringent timing and safety constraints --- hence, a security mechanism must not violate these requirements. There has also been a lot of interest in using multicore processors in RTS, opening up pressing security challenges. Hence, the focus of this proposal is to
...
The proposed techniques will be evaluated on various platforms --- from simulation engines to real hardware (automated rover and a scale-down robot testbed). The proposed research activities will be complemented with tightly integrated educational components reaching from undergraduate/graduate students, students from minority communities, and K-12 students.
...
This proposal addresses a fundamentally challenging problem, \ie how do we integrate security monitoring techniques into multicore RTS without affecting timing requirements? The focus of this research is to develop strategies that will allow us to trade back and forth between these seemingly conflicting properties so that designers of the systems can correctly gauge system requirements —-- hence, perhaps, meeting both the real-time constraints as well as integrating effective security mechanisms.  The proposed work will bridge the knowledge gap by analyzing the trade-offs between the two requirements --- whether reducing the promises made to one (say a reduction of the security goals) will improve the other (better real-time guarantees). 
%The outcomes of this work will enable researchers and system engineers
 ...
 to make well-informed choices and develop inherently safer and more secure systems.
