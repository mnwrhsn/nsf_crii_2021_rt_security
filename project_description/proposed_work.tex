...
To address these issues, we will \ca devise design-time analytical models to ensure that timing guarantees are met and \cb develop necessary APIs, libraries, and OS patches to deploy reactive security protocols into commodity real-time kernels. In particular, we will extend existing multi-mode real-time scheduling techniques~\cite{nelis2009two, yomsi2010scheduling, baek2020response} with security requirements and explore various design alternatives that mentioned above.  The development of our proposed reactive monitoring framework also requires system-level modifications. 
...
Our scheduler plug-ins and OS-level mechanisms will ensure that security tasks are executed in a tamper-proof environment (TrustZone enclaves) and carried out their desired functionalities. In the past we \textit{successfully used off-the-shelf trusted modules} in multiple cyber-physical use-cases~\cite{mhasan_resecure_iot, mhasan_iotsnp19} --- we will leverage our expertise on TEEs in the proposed research. We will \textit{open-source our implementation and release the APIs} for community use.
...
\begin{itemize}[leftmargin=*]
	\parskip 0pt
	\itemsep 0pt
	\item \rtask Extend multi-mode framework for multicore RTS --- this includes: \ca design-space exploration of multiple mode-switch alternatives, \cb integrate with RTOS and multicore schedulers, and \cc analyze the efficacy of each of the techniques.
